...
def create_architecture_small_cnn():
    ...
    model = Sequential()
    model.add(keras.layers.Conv2D(32, (3, 3), activation='relu', input_shape=(EFFICIENTNET_IMAGE_SIZE, EFFICIENTNET_IMAGE_SIZE, 3)))
...
def create_architecture_regression():
    """Small cnn from scratch.."""
    model = Sequential()
    model.add(keras.layers.Dense(28, activation='relu', input_shape=(EFFICIENTNET_IMAGE_SIZE, EFFICIENTNET_IMAGE_SIZE, 3)))
    model.add(keras.layers.Flatten())
    model.add(keras.layers.Dropout(0.3))
    model.add(keras.layers.Dense(32, activation='relu'))
    model.add(keras.layers.Dense(N_CLASSES, activation = ('softmax')))
...
def main(argv):
    ...
    trained_model  = train_model(FLAGS.base_dir, FLAGS.collection_id, model, X_train, y_train_cat)
    # Save checkpoint.
    print(
