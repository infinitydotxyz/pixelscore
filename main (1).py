...
def get_scores_collection(X_train, ids):
    ...
    scaler = MinMaxScaler(feature_range = (PIXELSCORE_SCALING_MIN,PIXELSCORE_SCALING_MAX))
    scores = scaler.fit_transform(scores.reshape(-1, 1))
    print('Scores scaled to ({},{})  {}'.format(PIXELSCORE_SCALING_MIN,PIXELSCORE_SCALING_MAX, scores))
    df = pd.DataFrame()
    df['id'] = ids
